...
function showGenerateModal(hash, yaml) {
    document.getElementById('workflowCommand').textContent = `npx claude-plugins@latest --workflow:#${hash}`;
...
function generateInstallCommand(component) {
    return `npx claude-plugins@latest --${component.type} "${component.path}"`;
}
...
function viewOnGitHub(path) {
    const url = `https://github.com/lifangda/claude-plugins/tree/main/${path}`;
