...
class PluginPageManager {
    ...
    renderPluginDetails() {
        ...
        document.getElementById('installPluginCmd').textContent = `/plugin install ${plugin.name}@claude-plugins`;
    ...
    generateComponentItems(componentNames, componentType, pluralType) {
        ...
        return componentNames.map((name, index) => {
            ...
            const installCommand = `npx claude-plugins@latest --${componentType} ${name}`;
