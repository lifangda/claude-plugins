# Testing Guide for claude-plugins

This guide explains how to test the `claude-plugins` CLI tool before publishing.
...
claude-plugins --dry-run
claude-plugins --language javascript-typescript --framework react --yes
claude-plugins --language common --yes

# Test in different directories
mkdir test-project && cd test-project
claude-plugins --language javascript-typescript --framework vue --yes
ls -la  # Check created files

# Cleanup
npm unlink -g claude-plugins
```
...
1. **Interactive Flow**: Start `claude-plugins` without flags and go through the full interactive setup
...
npx claude-plugins@latest --version
npx claude-plugins@latest --help
```
...
claude-plugins --language javascript-typescript --framework react --dry-run --yes --verbose
```
