...
const GLOBAL_AGENTS_DIR = path.join(os.homedir(), '.claude-plugins');
...
async function listGlobalAgents(options = {}) {
  ...
  try {
    ...
    if (allAgents.length === 0) {
      ...
      console.log(chalk.gray('💡 Create one with: npx claude-plugins@latest --create-agent <agent-name>'));
      return;
    }
    ...
    console.log(chalk.gray('  • List agents: npx claude-plugins@latest --list-agents'));
    console.log(chalk.gray('  • Remove agent: npx claude-plugins@latest --remove-agent <name>'));
...
async function findAgentUrl(agentName) {
  try {
    ...
    const rootUrl = `https://raw.githubusercontent.com/lifangda/claude-plugins/main/cli-tool/components/agents/${agentName}.md`;
    ...
    const categoriesResponse = await fetch('https://api.github.com/repos/lifangda/claude-plugins/contents/cli-tool/components/agents');
    ...
    for (const item of contents) {
      if (item.type === 'dir') {
        const categoryUrl = `https://raw.githubusercontent.com/lifangda/claude-plugins/main/cli-tool/components/agents/${item.name}/${agentName}.md`;
        try {
...
async function showAvailableAgents() {
  ...
  try {
    ...
    console.log(chalk.gray('  npx claude-plugins@latest --create-agent api-security-audit'));
    console.log(chalk.gray('  npx claude-plugins@latest --create-agent deep-research-team/academic-researcher'));
