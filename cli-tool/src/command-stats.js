...
function displayCommandStats(analysis) {
  ...
  if (!analysis.exists) {
    ...
    console.log(chalk.blue('\n💡 Run the setup first: npx claude-plugins'));
...
async function promptSetupWhenNoCommands(targetDir) {
  ...
  try {
    ...
    if (!setupNow) {
      console.log(chalk.yellow('⏭️  Setup skipped. Run "npx claude-plugins" anytime to set up your project.'));
      return false;
    }
    ...
    console.log(chalk.blue('💡 You can run setup manually with: npx claude-plugins'));
    return false;
  }
}
